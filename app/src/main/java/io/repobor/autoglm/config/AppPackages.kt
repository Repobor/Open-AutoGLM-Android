 ...
 * Ported from Open-AutoGLM Python project.
 */
object AppPackages {

    /**
     * Comprehensive mapping of app display names to Android package names.
    ...
    private val APP_MAPPINGS = mapOf(
        ...
        "WhatsApp" to "com.whatsapp",
        "AutoGLM" to "io.repobor.autoglm"
    )

    /**
     * Get the package name for an app by its display name.
     * @param appName The display name of the app (case-sensitive)
     * @return The Android package name, or null if not found
     */
    fun getPackageName(appName: String): String? {
        return APP_MAPPINGS[appName]
    }

    /**
     * Get the app display name from its package name.
     * Returns the first matching display name if multiple names map to the same package.
     * @param packageName The Android package name
     * @return The display name of the app, or null if not found
     */
    fun getAppName(packageName: String): String? {
        return APP_MAPPINGS.entries.firstOrNull { it.value == packageName }?.key
    }

    /**
     * Get a list of all supported app names.
     * @return List of all registered app display names
     */
    fun listSupportedApps(): List<String> {
        return APP_MAPPINGS.keys.toList()
    }

    /**
     * Get a list of all supported package names (unique).
     * @return List of unique package names
     */
    fun listSupportedPackages(): List<String> {
        return APP_MAPPINGS.values.distinct()
    }

    /**
     * Check if an app name is supported.
     * @param appName The display name to check
     * @return true if the app is supported, false otherwise
     */
    fun isSupported(appName: String): Boolean {
        return APP_MAPPINGS.containsKey(appName)
    }

    /**
     * Check if a package name is supported.
     * @param packageName The package name to check
     * @return true if the package is supported, false otherwise
     */
    fun isPackageSupported(packageName: String): Boolean {
        return APP_MAPPINGS.containsValue(packageName)
    }

    /**
     * Get all app names that map to a given package name.
     * Useful when multiple names (e.g., Chinese and English) map to the same package.
     * @param packageName The Android package name
     * @return List of all display names for this package
     */
    fun getAllAppNamesForPackage(packageName: String): List<String> {
        return APP_MAPPINGS.filter { it.value == packageName }.keys.toList()
    }
}
